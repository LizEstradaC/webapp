...
function savecompartir(req , res){
	...
	Publication.findById(req.params.id, function(error, publication){
      if(error){
        return res.status(500).send({message: 'Error al guardar la like'});
      }else{

	 newpublication.text = publication.text;
	 newpublication.file = publication.file;
	 newpublication.liked = 0;
	 newpublication.created_at = moment().unix();
	 newpublication.user = publication.user;
	 newpublication.compartir = req.user.sub;
         
         newpublication.save(function(error, documento){
            if(error){
               return res.status(500).send({message: 'Error al guardar la like'});
            }else{ 
	...
	Publication.findById(req.params.id, function(error, documento){
      if(error){
        return res.status(500).send({message: 'Error al guardar la like'});
         ...
         publication.liked = publication.liked+1;
...
function getPublications(req, res){
	...
	Follow.find({user: req.user.sub}).populate('followed').exec((err, follows) => {
		...
		Publication.find({user: {"$in": follows_clean}}).sort('-created_at').populate('user').populate('compartir').populate('comment').populate('comment.commented').paginate(page, itemsPerPage, (err, publications, total) => {
...
function getPublicationsUser(req, res){
	...
	Publication.find({user: user}).sort('-created_at').populate( 'user compartir').paginate(page, itemsPerPage, (err, publications, total) => {
		if(err) return res.status(500).send({message: 'Error devolver publicaciones'});
